...
import { Search, Plus, Minus, ShoppingCart, Send, ChevronDown, X, Maximize2, ListFilter, Check } from 'lucide-react';
...
const WaiterPage: React.FC<WaiterPageProps> = ({ currentUser, onSendOrder, menuItems }) => {
  ...
  const updateCart = (item: MenuItem, delta: number) => {
    ...
    setCart(prev => {
      ...
      if (delta > 0) return [...prev, { id: Date.now().toString(), menuItem: item, quantity: 1 }];
      return prev;
    });
  };
  ...
  return (
    <div className="flex h-full flex-col lg:flex-row overflow-hidden bg-slate-50 relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto pb-32 lg:pb-0">
        <div className="p-4 lg:p-6 bg-white/80 backdrop-blur-xl border-b sticky top-0 z-20">
          <div className="flex items-center gap-3 max-w-4xl mx-auto w-full relative">
            <div className="relative flex-1">
              ...
              <input 
                ...
                autoComplete="off"
                className="w-full pl-11 pr-4 py-3.5 bg-slate-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-semibold placeholder transition-all text-sm:text-slate-400"
                value={searchTerm}
            ...
            <div className="relative" ref={categoryMenuRef}>
              <button 
                ...
                className={`p-3.5 rounded-2xl border-2 transition-all flex items-center justify-center relative ${
              ...
              {showCategoryMenu && (
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="p-3">
                    ...
                    {CATEGORIES.map(cat => (
                      <button
                        ...
                        className={`w-full text-left px-5 py-3.5 rounded-xl text-xs font-bold transition-all flex items-center justify-between group ${
          ...
          {activeCategory !== 'All' && (
            <div className="max-w-4xl mx-auto w-full mt-3 flex items-center gap-2">
               <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Kategori Aktif:</span>
               <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[9px] font-black uppercase flex items-center gap-2">
                 {activeCategory}
        ...
        <div className="p-4 lg:p-8 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 lg:gap-6">
          {filteredItems.map(item => {
            const inCart = cart.find(i => i.menuItem.id === item.id);
            return (
              <div key={item.id} className={`bg-white rounded-[2.5rem] border border-slate-200/60 overflow-hidden group shadow-sm flex flex-col transition-all duration-300 ${item.isSoldOut ? 'opacity-80 grayscale-[0.4]' : 'hover:shadow-2xl hover:-translate-y-1'}`}>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                  <div>
                    <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1 leading-none">{item.category}</p>
                  ...
                  <div className="pt-2">
                    {inCart ? (
                      ...
                      <button 
                        ...
                        className={`w-full py-3.5 rounded-[1.2rem] text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                          item.isSoldOut ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-800 text-white hover:bg-emerald-600 shadow-lg shadow-slate-100'
                        }`}
                      >
      ...
      {/* Order Sent Splash Screen */}
      {showSplash && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[300] flex items-center justify-center animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl p-12 shadow-2xl max-w-md w-full mx-4 animate-in zoom-in-95 duration-300 transform transition-all hover:scale-105">
            <div className="text-center space-y-6">
              ...
              <p className="text-emerald-600 font-bold text-sm uppercase tracking-widest">Pesanan sedang diproses</p>
              <div className="w-20 h-2 bg-slate-200 rounded-full mx-auto overflow-hidden">
                <div className="h-full bg-emerald-600 animate-pulse w-1/2" />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Image Zoom Modal */}
      {zoomImage && (
        <div className="fixed inset-0 bg-black/95 z-[200] flex flex-col animate-in fade-in duration-300">
          ...
          <div className="flex-1 flex items-center justify-center p-4">
            <img src={zoomImage.image} className="max-w-full max-h-[70vh] object-contain rounded-[2rem] shadow-2xl" />
          </div>
          <footer className="p-10 flex justify-center shrink-0">
             <button 
                ...
                className="px-12 py-5 bg-emerald-600 text-white rounded-full font-black text-sm uppercase tracking-widest hover:bg-emerald-700 active:scale-95 transition-all shadow-2xl shadow-emerald-500/20"
             >
      ...
      <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-40 transition-all duration-500 ease-in-out ${cart.length > 0 ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <div className={`bg-white/90 backdrop-blur-2xl border-t border-white/40 shadow-[0_-15px_40px_-15px_rgba(0,0,0,0.3)] rounded-t-[3rem] transition-all duration-500 ${isCartExpanded ? 'h-[85vh]' : 'h-24'} flex flex-col relative`}>
          ...
          <div className={`flex-1 flex flex-col overflow-hidden transition-opacity duration-300 ${isCartExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className="px-8 pb-6 border-b border-slate-100 flex justify-between items-center">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              <div className="space-y-3">
                ...
                <input id="table-info" name="table-info" type="text" placeholder="Nomor/Meja" autoComplete="off" className="w-full px-6 py-5 bg-slate-100 border-0 rounded-[1rem] focus:ring-2 focus:ring-emerald-500 text-xl font-black text-slate-800 placeholder:text-slate-200 uppercase" value={tableInfo} onChange={e => setTableInfo(e.target.value.toUpperCase())} />
              </div>
              <div className="space-y-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Daftar Order</p>
                <div className="space-y-3">
                  {cart.map(item => (
                    <div key={item.id} className="flex justify-between items-center bg-white p-4 rounded-[1rem] border border-slate-100 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <textarea id="order-notes" name="order-notes" placeholder="Contoh: Sambal pisah, Tanpa es, ..." className="w-full px-6 py-5 bg-slate-100 border-0 rounded-[1rem] text-sm h-32 resize-none" value={notes} onChange={e => setNotes(e.target.value)} />
              </div>
            </div>
            <div className="p-8 bg-white/50 border-t border-slate-100 backdrop-blur-md">
              ...
              <button disabled={cart.length === 0 || !tableInfo} onClick={handleSubmit} className="w-full py-6 bg-emerald-600 text-white rounded-[2.5rem] font-black uppercase text-sm tracking-[0.1em] flex items-center justify-center gap-3 shadow-2xl shadow-emerald-200 hover:bg-emerald-700 active:scale-95 disabled:grayscale transition-all"><Send className="w-5 h-5" /> Kirim Pesanan</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:flex w-[450px] bg-white border-l shadow-2xl flex-col h-full shrink-0 z-30">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10">
          <div className="space-y-3">
            ...
            <input id="order-identity" name="order-identity" type="text" placeholder="PEMESAN" autoComplete="off" className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-[2rem] focus:outline-none focus:ring-2 focus:ring-emerald-500 text-xl font-black uppercase tracking-tighter" value={tableInfo} onChange={e => setTableInfo(e.target.value.toUpperCase())} />
          </div>
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              {cart.length === 0 && <div className="py-12 text-center text-slate-300 font-bold uppercase tracking-widest text-xs border-2 border-dashed border-slate-100 rounded-[2rem]">Keranjang Kosong</div>}
            </div>
          </div>
          <div className="space-y-3">
        ...
        <div className="p-8 bg-emerald-50/50 border-t border-emerald-100 space-y-8">
          ...
          <button disabled={cart.length === 0 || !tableInfo} onClick={handleSubmit} className="w-full py-6 bg-emerald-600 text-white rounded-[2.5rem] font-black uppercase text-sm tracking-[0.1em] flex items-center justify-center gap-3 shadow-2xl shadow-emerald-200 hover:bg-emerald-700 active:scale-95 disabled:grayscale transition-all"><Send className="w-5 h-5" /> Kirim Ke Admin</button>
        </div>
      </div>
    </div>
  );

...
const WaiterPage: React.FC<WaiterPageProps> = ({ currentUser, onSendOrder, menuItems }) => {
  ...
  return (
    <div className="flex h-full flex-col lg:flex-row overflow-hidden bg-slate-50 relative">
          ...
          <div className="flex items-center gap-3 max-w-4xl mx-auto w-full relative">
            <div className="relative flex-1">
              ...
              <input 
                ...
                autoComplete="off"
                className="w-full pl-11 pr-4 py-2 bg-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-semibold placeholder transition-all text-sm:text-slate-400"
                value={searchTerm}
            ...
            <div className="relative" ref={categoryMenuRef}>
              ...
              {showCategoryMenu && (
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="p-2">
                    ...
                    {CATEGORIES.map(cat => (
                      <button
                        ...
                        className={`w-full text-left px-5 py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-between group ${
            ...
            <div key={group.id}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 lg:gap-6">
                {items.map(item => {
                  ...
                  return (
                    <div key={item.id} className={`bg-white rounded-xl border border-slate-200/60 overflow-hidden group shadow-sm flex flex-col transition-all duration-300 ${item.isSoldOut ? 'opacity-80 grayscale-[0.4]' : 'hover:shadow-2xl hover:-translate-y-1'}`}>
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                        <div>
                          ...
                          <p className="font-black text-slate-600 text-xs mt-1">Rp {item.price.toLocaleString('id-ID')}</p>
                        </div>
      ...
      {showSplash && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-300 flex items-center justify-center animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-12 shadow-2xl max-w-md w-full mx-4 animate-in zoom-in-95 duration-200 transform transition-all hover:scale-105">
            ...
            <div ref={formContainerRef} className="flex-1 overflow-y-auto p-8 space-y-8 pb-32">
              <div className="space-y-3">
                ...
                <input 
                  ...
                  className="w-full px-6 py-3 bg-slate-100 border-0 rounded-2xl focus:ring-2 focus:ring-emerald-500 text-xl font-black text-slate-800 placeholder:text-slate-200 uppercase" 
                  value={tableInfo} 
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  ...
                  className="w-full px-6 py-5 bg-slate-100 border-0 rounded-2xl text-sm h-32 resize-none titlecase" 
                  value={notes} 
      ...
      <div className="hidden lg:flex w-112.5 bg-white border-l shadow-2xl flex-col h-full shrink-0 z-30">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10">
          <div className="space-y-3">
            ...
            <input id="order-identity" name="order-identity" type="text" placeholder="PEMESAN" autoComplete="off" className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-4xlcus:outline-none focus:ring-2 focus:ring-emerald-500 text-xl font-black uppercase tracking-tighter" value={tableInfo} onChange={e => setTableInfo(e.target.value.toUpperCase())} />
          </div>
          <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <textarea id="special-requests" name="special-requests" placeholder="Permintaan spesial..." className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-4xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm h-32 resize-none" value={notes} onChange={e => setNotes(e.target.value)} />
          </div>
        </div>
